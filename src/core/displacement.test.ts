...
describe('glassFilterMarkup filter region', () => {
  it('uses a fixed 170% objectBoundingBox region at every size (no resize flash)', () => {
    const small = glassFilterMarkup('r1', {
      width: 100,
      height: 100,
      bezel: 14,
      scale: 40,
    ...
    const large = glassFilterMarkup('r2', {
      width: 800,
      height: 400,
      bezel: 14,
      scale: 46,
    ...
    for (const m of [small, large]) {
      expect(m).toContain('x="-35%"')
      expect(m).toContain('y="-35%"')
      expect(m).toContain('width="170%"')
      expect(m).toContain('height="170%"')
